          ...
          <div key={i.id} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm space-y-2">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-gray-500">{i.tecnico || 'Sin técnico'} · {i.fecha_instalacion || 'Sin fecha'}</p>
              </div>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 ${estadoColors[i.estado] || 'bg-gray-100 text-gray-600'}`}>{i.estado}</span>
            </div>
